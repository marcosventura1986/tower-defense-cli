...
void Enemy::Activate(const Vector2D& startPosition) {
...
// Setters
void Enemy::SetPosition(const Vector2D position) { this->position = position; }
void Enemy::SetTarget(const Vector2D target) { this->target = target; }

void Enemy::SetVelocity(const double velocity) { this->velocity = velocity; }
...
Vector2D Enemy::CalculateNewPosition() const {
